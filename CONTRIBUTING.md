...
The test suite lives in `test/` and is excluded from the npm tarball by the `files` allowlist in
`package.json`. Plugin manifests, marketplace catalogs, assets, and CI configuration are part of
the supported distribution surface.
...
See [TECHNICAL.md](TECHNICAL.md) for architecture, the tool contract, and local development.

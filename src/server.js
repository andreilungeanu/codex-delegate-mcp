...
export function buildServer({
  ...
  server.registerTool(
    ...
    {
      description:
        "Delegate a coding task to the OpenAI Codex CLI. Never shell out to codex — use this tool. Pass a precise brief in spec. Defaults: mode=agent, network=false. Plan workflow: mode=plan then resume with mode=agent and resumeThreadId. Returns the authoritative final message, thread id, changed files, and warnings. See the delegate skill for orchestration.",
      inputSchema: {
        spec: z
          .string()
          .describe(

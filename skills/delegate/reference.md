...
| `reasoningEffort` | `xhigh` | `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. `ultra` is `gpt-5.6-sol` and `gpt-5.6-terra` only; `gpt-5.6-*` reject `minimal`; older models reject `none`. A rejected value fails the turn, and the model's own message names what it takes. |

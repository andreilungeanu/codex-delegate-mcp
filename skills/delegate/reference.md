...
| `reasoningEffort` | `xhigh` | `none`, `low`, `medium`, `high` and `xhigh` run on every model listed here. `max` and `ultra` need a `gpt-5.6-*` model; `ultra` sends `max` with the CLI's own task delegation on top. A rejected value fails the turn, and the model's own message names what it takes. |

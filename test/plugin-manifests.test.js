...
test("Codex plugin manifest references real portable components", () => {
  ...
  assert.equal(manifest.name, "codex-delegate-mcp");
  ...
  assert.equal(manifest.mcpServers["codex-delegate-mcp"].command, "npx");
...
test("marketplaces and Copilot plugin point at the intended package", () => {
  ...
  assert.equal(codexEntry.name, "codex-delegate-mcp");
